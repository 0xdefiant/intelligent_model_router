...
${input}`,
      ...
      console.error('[Groq] extractExpenseData error:', err);
      throw err;
    }
  }
...
Recent context: ${JSON.stringify(context.slice(0, 10))}`,
      ...
      return { explanation: response.choices[0].message.content || '', confidence: 0.8 };
    } catch (err) {
      console.error('[Groq] analyzeAnomaly error:', err);
      throw err;
    }
  }
...
${policyText}`,
      ...
      console.error('[Groq] evaluatePolicy error:', err);
      throw err;
    }
  }

  estimateCost(inputTokens: number, outputTokens: number): number {
    return (inputTokens / 1000) * 0.00005 + (outputTokens / 1000) * 0.00008;
  }
}
